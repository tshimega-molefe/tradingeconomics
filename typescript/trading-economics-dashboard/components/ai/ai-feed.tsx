...
import { type AI } from "@/server/actions/generate-ai-search-result";
...
import { ChatScrollAnchor } from "@/hooks/use-chat-scroll-anchor";
import { useEnterSubmit } from "@/hooks/use-enter-submit";
import { Message, SearchResult, StreamMessage } from "@/types/ai-feed";
import Textarea from "react-textarea-autosize";
import FollowUpComponent from "./response/follow-up";
import ImagesComponent from "./response/image-response";
import LLMResponseComponent from "./response/model-response";
import SearchResultsComponent from "./response/search-results";
import UserMessageComponent from "./response/user-message";
import VideosComponent from "./response/video-response";

const AIFeedPage = ({}) => {
  // 3. Set up action that will be used to stream all the messages
  const { GenerateSearchResult } = useActions<typeof AI>();
  ...
  const handleUserMessageSubmission = async (
    ...
    try {
      const streamableValue = await GenerateSearchResult(userMessage);
